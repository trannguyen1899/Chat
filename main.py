...
def handle_query_input(user_question):
    """
    GENERATE RESPONSE
    Return source documents and display on the screen following the html template
    """
...
def main():
    ...
    # After inputing the question, user MUST click the button "Go" to start generating the response
    # ============================================================================================
    st.header("Chat with multiple PDFs :books:")

    # QUERY INPUT   
    container = st.container()
    col1, col2 = st.columns([0.9, 0.1]) # The text input widget and a button Go to enter the question
    with col1:
        user_question = st.text_input("Ask a question about your documents:")
    with col2:
        st.markdown('<p style="font-family:Courier; color:White; font-size: 8px;">H</p>', unsafe_allow_html=True)
        if st.button('Go'):
            handle_query_input(user_question)
    ...
    with st.sidebar:
        ...
        # HOW TO USE, note on HOW TO CHOOSE APPRORIATE LLM MODEL, ABOUT US
        sidebar()
